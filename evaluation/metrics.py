...
def dice_coefficient(preds: Tensor,
    ...
    return (2. * preds.eq(labels).long().sum((1, 2)) + smoothing_coeff) /\
           (labels.sum((1, 2)) + preds.sum((1, 2)) + smoothing_coeff)
...
def pixel_accuracy(preds: Tensor,
    ...
    return (preds.eq(labels).long().sum((1, 2)) + smoothing_coeff) /\
           (labels.shape[1] * labels.shape[2] + smoothing_coeff)
